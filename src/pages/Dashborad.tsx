...
export const Dashborad = () => {
    ...
    const filteredContent = content.filter(item => {
        ...
        if (item.tags && Array.isArray(item.tags)) {
            if (item.tags.length > 0) {
                ...
                if (typeof item.tags[0] === 'string') {
                    ...
                    tags = (item.tags as any[]).map((tagItem: any) => {
                        if (typeof tagItem === 'object' && 'tag' in tagItem && tagItem.tag && 'title' in tagItem.tag) {
        ...
        const matchesSearch = item.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
            tags.some(tagTitle => tagTitle.toLowerCase().includes(searchQuery.toLowerCase()));

        const matchesType = typeFilter === "all" || 
            item.type.toLowerCase() === typeFilter.toLowerCase();
        const isRecent = currentFilter === "recent" && item.createdAt ? 
            new Date(item.createdAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) : 
            true;

        return matchesSearch && matchesType && (currentFilter === "all" || isRecent);
    });
    ...
    return (
        <div className="flex h-screen bg-gray-900">
            ...
            <div className="flex-1 ml-64 overflow-y-auto">
                <div className="p-8">
                    <div className="mb-8">
                        <h1 className="text-3xl font-bold text-white mb-2">Your Second Brain</h1>
                        <p className="text-gray-400">Store and organize your important links in one place</p>
                    </div>
                    <div className="mb-8">
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => { setTypeFilter('all'); setCurrentFilter('all'); }}
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-64">
                            <p className="text-gray-400 mb-4">
                                {searchQuery ? 'No content found matching your search.' : 'No content yet. Start by adding your first link!'}
                            </p>
                            {!searchQuery && (
                                <button
                                    onClick={() => setShowCreateModal(true)}
                            ...
                            {filteredContent.map((item) => {
                                ...
                                if (item.tags && Array.isArray(item.tags) && item.tags.length > 0) {
                                    ...
                                    if (typeof item.tags[0] === 'string') {
                                        ...
                                        tagsForCard = (item.tags as string[]).map(tag => ({ title: tag }));
                                    } else {
                                        
                                        tagsForCard = (item.tags as any[]).map(tagItem => {
                                            if ('tag' in tagItem && tagItem.tag && 'title' in tagItem.tag) {
